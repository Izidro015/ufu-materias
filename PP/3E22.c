...
int main(){
    ...
    while (a<= 1000){
        ...
        dez = (produto/10)%10;
        uni = produto%10;
        if ((cemmil == uni) && (dez == dezmil) && (mil == cem)){//condicional para verificar se o numero é palindromo
            palindromo = produto;
        }
        b++;
        }
        if (result < palindromo){ //condicional para achar o maior valor de palindromos.
            result = palindromo;
        }
        b = 100;
        a++;
